 ...
 */
export async function searchParts(query: string, limit = 50, offset = 0): Promise<CatalogEntry[]> {
  const catalog = await loadPartsCatalog();
  if (!query.trim()) return catalog.slice(offset, offset + limit);
  ...
  return results.slice(offset, offset + limit).map(r => r.entry);
}
